...
function renderTransactionList(transactions) {
  ...
  if (!list) return;
  if (transactions.length === 0) { list.innerHTML = `<div class="empty-state"><i class="fa fa-inbox"></i><p>No transactions found.</p></div>`; return; }
...
function renderAnalyticsCharts() {
  ...
  const catMap = {};
  monthExpenses.forEach(t => { catMap[t.category || 'Other'] = (catMap[t.category || 'Other'] || 0) + Math.abs(t.amount); });
  ...
  categoryChartInstance = new Chart(document.getElementById('categoryChart').getContext('2d'), {
    type: 'bar',
    data: { labels: catLabels.length ? catLabels : ['No Data'], datasets: [{ data: catData.length ? catData : [0], backgroundColor: colors.slice(0, catLabels.length || 1), borderRadius: 8, borderSkipped: false }] },
    options: { responsive: true, indexAxis: 'y', plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ₹${ctx.raw.toFixed(2)}` } } }, scales: { x: { ticks: { color: '#94a3b8', callback: v => `₹${v}` }, grid: { color: 'rgba(255,255,255,0.05)' } }, y: { ticks: { color: '#94a3b8' }, grid: { display: false } } } }
  });
}
...
function renderBudgets(budgets) {
  ...
  list.innerHTML = budgets.map(b => {
    ...
    const remainingText = b.overspent ? `Over by ₹${Math.abs(b.remaining).toFixed(0)}` : `₹${b.remaining.toFixed(0)} left`;
    return `
      <div class="budget-card">
        ...
        <div class="budget-bar"><div class="budget-bar-fill ${statusClass}" style="width:${b.percent}%"></div></div>
  ...
  // Load AI warnings for overspent budgets
...
async function deleteBudget(id) {
  const token = localStorage.getItem('token');
  try {
    const res = await fetch(`${API_URL}/budget/${id}`, { method: 'DELETE', headers: { 'Authorization': token } });
...
function closeBudgetModal() { document.getElementById('budget-modal-overlay').classList.add('hidden'); document.getElementById('budget-category').value = ''; document.getElementById('budget-limit').value = ''; }
