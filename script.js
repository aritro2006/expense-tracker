...
function quickParse(text) {
  ...
  const incomeWords = ['received','got','earned','salary','income','bonus','refund','credit',
    'deposited','freelance','stipend','allowance','pocket money','pocket','awarded','won',
    'prize','gift','cashback','reimbursement','scholarship','commission','dividend'];
  ...
  return { text: description, amount, type: hasIncome ? 'income' : 'expense' };
}
...
async function parseSmartInput() {
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      if (data.text   && data.text.length > 1)  document.getElementById('t-text').value   = data.text;
      if (data.amount && data.amount > 0)        document.getElementById('t-amount').value = Math.abs(data.amount);
      if (data.type === 'income' || data.type === 'expense') setType(data.type);
    }
  } catch {}
  finally { btn.innerHTML = '<i class="fa fa-wand-magic-sparkles"></i>'; btn.disabled = false; }
}
...
function renderCharts(transactions) {
  ...
  barChartInstance = new Chart(document.getElementById('barChart').getContext('2d'), {
    ...
    data: { labels: ['Income','Expenses'],
      datasets:[{ data:[income,expense], backgroundColor:['rgba(16,185,129,.7)','rgba(239,68,68,.7)'],
  ...
  donutChartInstance = new Chart(document.getElementById('donutChart').getContext('2d'), {
    type: 'doughnut',
    data: { labels:['Income','Expenses'],
      datasets:[{ data:[income||0.001,expense||0.001], backgroundColor:['rgba(16,185,129,.8)','rgba(239,68,68,.8)'],
...
function renderAnalyticsCharts() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    data.push(allTransactions
      .filter(t => { const td = new Date(t.date); return td.getMonth()===d.getMonth() && td.getFullYear()===d.getFullYear() && t.amount<0; })
      .reduce((s,t) => s + Math.abs(t.amount), 0));
  }
  trendChartInstance = new Chart(document.getElementById('trendChart').getContext('2d'), {
    type: 'line',
    data: { labels, datasets:[{ label:'Expenses', data, borderColor:'#7c3aed',
        backgroundColor:'rgba(124,58,237,.1)', borderWidth:2.5,
        pointBackgroundColor:'#7c3aed', pointRadius:4, fill:true, tension:.4 }] },
  ...
  categoryChartInstance = new Chart(document.getElementById('categoryChart').getContext('2d'), {
    type: 'bar',
    data: { labels: catLabels.length ? catLabels : ['No Data'],
    ...
    options:{ responsive:true, indexAxis:'y',
      ...
      scales:{ x:{ticks:{color:'#94a3b8',callback:v=>`₹${v}`}}, y:{ticks:{color:'#94a3b8'},grid:{display:false}} } }
  });
...
async function loadHealthScore() {
  ...
  } catch { document.getElementById('health-score-breakdown').textContent = 'Score unavailable.'; }
}
...
function closeGoalModal() {
  ...
  ['goal-title','goal-target','goal-saved','goal-deadline'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = id === 'goal-saved' ? '0' : id === 'goal-emoji' ? '🎯' : '';
  });
...
function renderRecurring(items) {
  ...
  if (!items.length) {
    list.innerHTML = `<div class="empty-state"><i class="fa fa-rotate"></i><p>No recurring transactions yet. Add one to auto-track monthly items.</p></div>`;
    return;
  }
...
function showTyping() {
  ...
  div.innerHTML = `
    <div class="ai-msg-avatar" style="background:rgba(124,58,237,.2);color:var(--accent2);width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.72rem;flex-shrink:0">
